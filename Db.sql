...
BEGIN
    IF :NEW.COMMISSIONS_ID IS NULL THEN
        SELECT EDUARDED.COMMISSIONS_SEQ.NEXTVAL INTO :NEW.COMMISSIONS_ID FROM DUAL;
    END IF;
END;
...
BEGIN
    OPEN v_cursor FOR
        SELECT ps.name AS NAME_PURCHASING_STATUS, t.discount, u.name AS USER_NAME, u.surname, u.email, u.phone, d.carrer, d.street, d.postal_code, 
        d.city, de.name AS DEPARTMENT_NAME, pa.number_election, p.name AS PRODUCT_NAME, p.price, p.content, 
        be.name AS BANK_ENTITY_NAME, tp.units
        FROM TRANSACTIONS t
        INNER JOIN TRANSACTIONPRODUCT tp ON t.transaction_id = tp.transaction_id
